-- Function to expose auth status for parents to admins only
...
begin
  ...
  left join users_by_email ube on lower(ube.email) = lower(p.email)
  left join providers pr on pr.user_id = ube.user_id;
end;
$$;