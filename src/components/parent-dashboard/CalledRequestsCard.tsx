...
const CalledRequestsCard: React.FC<CalledRequestsCardProps> = ({
  ...
  // No additional filtering needed - all called requests belong to the current parent
        ...
        <div className="space-y-3">
          {calledRequests.map((request) => {
            ...
            return (
              <div 
                key={request.id}
                className="p-3 border rounded-md flex items-center gap-3 hover:shadow-md transition-shadow duration-300 bg-green-50 border-green-200 animate-pulse"
              >
                <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center border border-green-300 animate-bounce flex-shrink-0">
